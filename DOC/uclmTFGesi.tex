...
El consumo de energía juega un papel fundamental en el progreso y bienestar de la sociedad. Tanto es así que la demanda de energía se multiplica con el paso del tiempo y su precio aumenta. Además, las fuentes de energía fósiles son agotables y se debe apostar por el uso de energías renovables y el autoconsumo tanto como sea posible. Para gestionar de la forma mas eficiente esta multitud de fuentes de energía tenemos que tener en cuenta los factores que las condicionan y seleccionar la más adecuada en cada momento para satisfacer la demanda de energía. Este Trabajo Fin de Grado propone la creación de un sistema inteligente para que la gestión de energía en hogares se realice de una manera óptima y eficiente mediante la incorporación de nuevas fuentes de energía como son la instalación de módulos fotovoltaicos y una batería. La instalación se modela como un problema de satisfacción de restricciones (PSR) identificándose un conjunto de variables, el dominio de las mismas y un conjunto de restricciones entre valores de las variables que siempre vienen determinadas por la configuración de la instalación, por las propiedades físicas de generación y consumo de la energía, así como por las condiciones climáticas. A este PSR se le añade una función objetivo de coste energético que permite hallar la solución del mismo orientándolo a la minimización de dicha función mediante programación lineal. Esto permite el máximo aprovechamiento de la energía y da como resultado un coste económico mínimo para el hogar. Este sistema es usable mediante una aplicación web alojada en la nube de IBM Cloud Foundry y Amazon Web Services que permite interactuar a los usuarios con el sistema, configurando las características de su hogar y pudiendo realizar simulaciones de un día concreto. Gracias a los resultados de estas simulaciones, los usuarios pueden comparar la distribución energética y gasto económico que se hubiera producido empleando el sistema propuesto frente a lo ocurrido realmente con el consumo exclusivo de la red eléctrica.
...
Energy consumption plays an essential role in the development and well-being of society. So much so that the demand for energy multiplies with the passage of time and its price increases. Furthermore, fossil energy sources are exhaustible and we must opt for the use of renewable energies and self-consumption as much as possible. In order to manage this multitude of energy sources in the most efficient way, we have to take into account the factors that condition them and select the most appropriate one at each moment to satisfy the demand for energy. This project proposes the creation of an intelligent system so that energy management in homes is carried out in an optimal and efficient way through the incorporation of new energy sources such as the installation of photovoltaic modules and a battery. The installation is modelled as a restriction satisfaction problem (CSPs) identifying a set of variables, their domain and a set of restrictions between the values of the variables that are always determined by the configuration of the installation, by the physical properties of energy generation and consumption, as well as by climatic conditions. To this CSPs is added an objective energy cost function that allows the solution to be found, orienting it to the minimisation of this function by means of linear programming. This allows the maximum use of energy and results in a minimum economic cost for the home. This system is usable by means of a web application hosted in the IBM Cloud Foundry and Amazon Web Services that allows users to interact with the system, configuring the characteristics of their home and being able to carry out simulations of a specific day. Thanks to the results of these simulations, users can compare the energy distribution and economic expenditure that would have been produced using the proposed system against what really happened with the exclusive consumption of the electricity network.
\end{abstract}
%---
...
\printbibliography[heading=subbibliography,nottype=online,title={Otras fuentes}]
% -------------------------
