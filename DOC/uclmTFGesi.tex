...
\begin{abstract}
Energy consumption plays a fundamental role in the progress and well-being of society. The demand for energy multiplies over time and thus the price of energy. Furthermore, fossil energy sources are exhaustible and we must opt for the means of extracting renewable energies as much as possible. In order to be able to deal with this, it should be possible to obtain energy from numerous sources, depending on various factors that condition them and selecting the most appropriate one at each moment, without depending on a single energy source to satisfy demand as is currently the case. Unfortunately this is a difficult task for the human being, but possible for computing.\\

This end-of-grade work is focused in the creation of an intelligent system for managing energy in the home in the most optimal and efficient way possible. Depending on a given situation in each hour of the day, the system is able to model the amount of electrical energy received by each of the energy input sources to make the lowest economic cost in obtaining it. At the same time, the system models the amount of electrical energy supplied to a series of outputs, as all the energy generated must be consumed in one way or another, such as home consumption, charging a battery, etc.. This translates into an optimization and use of energy, which also results in economic savings in obtaining the necessary energy. In order to make this possible, a constraint satisfaction problem (CSPs) is modeled, the solutions of which are reached by means of linear programming, making use of scientific computing algorithms. In addition, it includes a web application that allows users to perform simulations of daily optimizations, hosted in the cloud of IBM Cloud Foundry and Amazon Web Services.
\end{abstract}
%---
