...
INDICATOR_SPOT = '613'  # Precio marginal del mercado intradiario (precio de compra/venta a la red elétrica)
...
def get_pvpc_price (start_date=time.strftime("20%y/%m/%d"),end_date=time.strftime("20%y/%m/%d")):
    global headers
...
def get_spot_price (start_date=time.strftime("20%y/%m/%d"),end_date=time.strftime("20%y/%m/%d")):
    global headers
...
if "__NAME__==__MAIN__":
    print ("PVPC: {} €/kwh\nSPOT: {} €/kwh".format(get_pvpc_price(),get_spot_price()))
